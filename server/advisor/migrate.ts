...
export async function ensureAdvisorTables(): Promise<void> {
  ...
  if (!process.env.DATABASE_URL) {
    console.warn("[advisor] DATABASE_URL is not set. Ownership Advisor tables were not created.");
    return;
  }
  ...
  try {
    ...
    for (const [key, value] of Object.entries(DEFAULT_SETTING_VALUES)) {
      await pool.query(
        `INSERT INTO advisor_admin_settings (key, value)
         VALUES ($1, $2::jsonb)
  ...
  migrated = true;
  console.log("[advisor] Ownership Advisor tables are ready.");
}

export function advisorMigrationStatements(): string[] {
  return [...STATEMENTS];
}
